...
function Index() {
  ...
  const forWhom = ["Para quem perdeu alguém especial.", "Para quem sente uma saudade que ainda dói.", "Para quem procura palavras de fé depois de uma despedida.", "Para quem deseja guardar a memória sem ficar preso à dor.", "Para famílias que estão aprendendo a continuar juntas.", "Para quem precisa lembrar que ainda existe esperança."];
  ...
  return <main className="overflow-hidden bg-background text-foreground">
    <section className="relative min-h-[94svh] bg-navy text-ivory">
      ...
      <div className="section-shell relative z-10 flex min-h-[94svh] flex-col justify-between py-6 sm:py-10">
        <header className="flex items-center justify-between border-b border-gold/30 pb-4">
          ...
          <a href="#o-livro" className="hidden items-center gap-2 text-xs uppercase tracking-[0.16em] text-ivory/70 sm:flex">Conhecer o livro <ArrowDown className="size-4"/></a>
        </header>
        <div className="grid items-center gap-10 py-12 lg:grid-cols-[1.15fr_.85fr]">
          <div className="max-w-2xl">
            ...
            <p className="mt-5 text-xs text-ivory/60">Pré-lançamento · Lançamento oficial em 1º de novembro · Tiragem limitada</p>
          </div>
    ...
    <section className="bg-navy py-24 text-ivory sm:py-32">
      <div className="section-shell grid items-center gap-14 lg:grid-cols-2">
        <div><Eyebrow>Antes de tudo, acolher</Eyebrow><h2 className="text-balance text-4xl sm:text-5xl">Há saudades que não desaparecem.</h2><div className="mt-7 space-y-4 text-base leading-8 text-ivory/70"><p>Algumas despedidas mudam a nossa maneira de olhar para a vida.</p><p>Ficam as lembranças. Ficam as perguntas. Ficam palavras que gostaríamos de ter dito. Ficam momentos que gostaríamos de reviver. E fica a saudade.</p></div><blockquote className="mt-8 border-l border-gold pl-6 text-2xl leading-snug text-gold">Você não precisa esquecer para continuar.<br/>Pode aprender a caminhar com a saudade.</blockquote></div>
    ...
    <section className="bg-ivory py-24 sm:py-32"><div className="section-shell"><div className="max-w-2xl"><Eyebrow>Um encontro possível</Eyebrow><h2 className="text-4xl text-navy sm:text-5xl">Talvez este livro seja para você.<br/>Ou para alguém que você ama.</h2></div><div className="mt-14 grid border-t border-gold/40 md:grid-cols-2">{forWhom.map((item,i)=><div key={item} className="flex gap-4 border-b border-gold/25 py-6 md:odd:border-r md:odd:pr-8 md:even:pl-8"><Heart className="mt-1 size-4 shrink-0 text-gold"/><p className="text-base leading-7 text-charcoal/75">{item}</p></div>)}</div></div></section>

    <section className="bg-background py-24 sm:py-32"><div className="section-shell grid items-center gap-14 lg:grid-cols-2"><img src={bibleLantern} alt="Livro aberto, vela e flores brancas" loading="lazy" width={1600} height={1072} className="aspect-[4/3] w-full object-cover"/><div><Eyebrow>Um livro para ser guardado</Eyebrow><h2 className="text-4xl text-navy sm:text-5xl">Há livros que você lê.<br/>E há livros que permanecem com você.</h2><p className="mt-7 leading-8 text-charcoal/70">Quando a dor aperta, algumas páginas podem ser revisitadas. Quando uma data especial chegar, algumas palavras podem ganhar outro significado. Quando a saudade voltar, você poderá abrir novamente este livro.</p><p className="mt-7 font-display text-2xl text-gold">Para ler, guardar, reler e compartilhar esperança.</p></div></div></section>

    <section className="bg-navy-soft py-24 text-ivory sm:py-32"><div className="section-shell grid gap-12 lg:grid-cols-[.7fr_1.3fr]"><div className="flex aspect-[4/5] items-center justify-center border border-gold/30 bg-navy"><div className="text-center"><Cross className="mx-auto size-10 text-gold" strokeWidth={1}/><p className="mt-5 font-display text-2xl">Pe. Wesley<br/>Xavier Ramos</p></div></div><div className="self-center"><Eyebrow>Sobre o autor</Eyebrow><h2 className="text-4xl sm:text-5xl">Presença, escuta, silêncio e fé.</h2><p className="mt-7 max-w-2xl text-lg leading-9 text-ivory/70">Sacerdote e presença pastoral junto a pessoas e famílias em momentos de dor e despedida, Pe. Wesley Xavier Ramos escreve a partir da escuta e do acompanhamento espiritual.</p><blockquote className="mt-9 border-l border-gold pl-6 text-3xl leading-snug text-gold">“Este livro não nasceu da tentativa de explicar a dor. Nasceu da experiência de permanecer ao lado de quem sofre.”</blockquote></div></div></section>

    <section id="pre-lancamento" className="bg-navy py-24 text-ivory sm:py-32"><div className="section-shell"><div className="grid items-center gap-14 border-y border-gold/40 py-14 lg:grid-cols-[1.2fr_.8fr]"><div><Eyebrow>Pré-lançamento</Eyebrow><h2 className="text-balance text-4xl sm:text-6xl">O primeiro exemplar pode ser seu antes do lançamento.</h2><p className="mt-7 max-w-2xl leading-8 text-ivory/70">O lançamento oficial acontecerá em 1º de novembro, véspera do Dia de Finados. O pré-lançamento é uma oportunidade de garantir antecipadamente seu exemplar da primeira tiragem, que é limitada.</p><div className="mt-9"><PreorderButton label="Garantir meu exemplar"/></div></div><div className="border border-gold/50 p-8 text-center"><p className="text-xs uppercase tracking-[0.2em] text-gold">Pré-lançamento</p><p className="my-5 font-display text-6xl text-ivory">01<span className="block text-2xl">de novembro</span></p><div className="gold-rule"/><p className="mt-5 text-xs uppercase tracking-[0.18em] text-ivory/60">Lançamento oficial · Tiragem limitada</p></div></div></div></section>

    <section className="bg-ivory py-24 sm:py-32"><div className="section-shell mx-auto max-w-3xl text-center"><Flame className="mx-auto size-8 text-gold" strokeWidth={1.3}/><h2 className="mt-7 text-balance text-4xl text-navy sm:text-5xl">Às vésperas do Dia de Finados, uma palavra de esperança.</h2><p className="mt-7 leading-8 text-charcoal/70">1º de novembro marca o lançamento oficial de um livro que nasceu para falar sobre aquilo que muitas famílias carregam no coração: a saudade de quem partiu. O Dia de Finados nos convida à memória, à oração e à esperança cristã.</p><p className="mt-5 leading-8 text-charcoal/70">Não para falar apenas sobre a morte. Mas para lembrar que, para quem crê, a história não termina na ausência.</p></div></section>

    <section className="bg-background py-24 sm:py-32"><div className="section-shell grid items-center gap-10 lg:grid-cols-[.3fr_.7fr]"><div className="flex justify-center"><Gift className="size-24 text-gold" strokeWidth={.7}/></div><div><Eyebrow>Um gesto de presença</Eyebrow><h2 className="text-balance text-4xl text-navy sm:text-5xl">Talvez você esteja procurando uma forma de dizer: “Estou aqui com você.”</h2><p className="mt-7 max-w-3xl leading-8 text-charcoal/70">Às vezes não sabemos o que dizer. Um livro pode ser uma forma silenciosa de presença. Uma lembrança. Um gesto. Uma palavra de fé.</p><div className="mt-8"><PreorderButton label="Quero garantir um exemplar"/></div></div></div></section>

    <section className="relative isolate bg-navy py-28 text-center text-ivory sm:py-40"><img src={horizon} alt="Horizonte iluminado sobre o mar" loading="lazy" width={1920} height={1088} className="absolute inset-0 -z-20 size-full object-cover"/><div className="absolute inset-0 -z-10 bg-navy/70"/><div className="section-shell"><AnchorMark/><h2 className="mx-auto mt-8 max-w-4xl text-balance text-4xl sm:text-6xl">Quando a saudade permanece,<br/><span className="font-script text-[1.2em] text-gold">a esperança</span> pode continuar iluminando o caminho.</h2><p className="mt-7 text-ivory/75">Garanta seu exemplar no pré-lançamento de <em>Quando a Saudade Permanece</em>.</p><div className="mt-9"><PreorderButton label="Garantir meu exemplar"/></div><p className="mt-5 text-xs uppercase tracking-[0.18em] text-ivory/60">Coleção Âncora · Livro I · 1º de novembro · Primeira tiragem</p></div></section>

    <footer className="bg-navy py-14 text-center text-ivory"><div className="section-shell"><Divider/><p className="mt-7 font-display text-2xl">Coleção Âncora</p><p className="mt-2 text-xs uppercase tracking-[0.18em] text-gold">Palavras de esperança para os caminhos da vida</p><p className="mt-8 text-sm text-ivory/55">Livro I · Quando a Saudade Permanece · Pe. Wesley Xavier Ramos</p><p className="mt-3 text-xs text-ivory/35">© 2026. Todos os direitos reservados.</p></div></footer>
