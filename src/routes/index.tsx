...
function Index() {
  ...
  return <main className="overflow-hidden bg-background text-foreground">
    ...
    <section className="bg-background py-24 sm:py-32"><div className="section-shell"><div className="mx-auto max-w-2xl text-center"><p className="mb-5 text-lg font-semibold uppercase tracking-[0.14em] text-gold sm:text-xl">O que você vai encontrar no livro</p><h2 className="text-2xl text-navy sm:text-3xl">Uma caminhada ao encontro da esperança.</h2></div><div className="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-5">{journey.map(([n,t,d])=><div key={n} className="group rounded-2xl border border-gold/30 bg-ivory p-7 text-center shadow-sm transition-all hover:-translate-y-1 hover:border-gold/60 hover:shadow-lg"><span className="mx-auto flex size-12 items-center justify-center rounded-full bg-gold/10 font-display text-xl text-gold transition-colors group-hover:bg-gold/20">{n}</span><h3 className="mt-5 text-xl text-navy">{t}</h3><p className="mt-3 text-sm leading-6 text-muted-foreground">{d}</p></div>)}</div></div></section>
