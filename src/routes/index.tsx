...
function Index() {
  ...
  return <main className="overflow-hidden bg-background text-foreground">
      ...
      <div className="section-shell relative z-10 flex min-h-[94svh] flex-col justify-between py-6 sm:py-10">
        ...
        <div className="grid items-center gap-10 py-12">
          <div className="max-w-2xl">
            ...
            <p className="mt-5 text-xs text-ivory/72">Pré-lançamento · Lançamento oficial em 17 de novembro</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <section className="relative isolate min-h-[680px] bg-charcoal py-24 text-ivory sm:py-32"><img src={contemplativeSea} alt="Mar e horizonte ao entardecer" loading="lazy" width={1600} height={1008} className="absolute inset-0 -z-20 size-full object-cover opacity-35"/><div className="absolute inset-0 -z-10 bg-charcoal/75"/><div className="section-shell"><div className="max-w-2xl"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-gold">O que você vai encontrar no livro</p><h2 className="mt-4 text-balance text-4xl sm:text-6xl">Uma caminhada ao encontro da esperança.</h2></div><div className="mt-14 grid gap-10 sm:grid-cols-2 lg:grid-cols-5">{journey.map(([n,t,d])=><div key={n}><span className="font-display text-3xl text-gold/70">{n}</span><h3 className="mt-3 text-xl">{t}</h3><p className="mt-2 text-sm leading-6 text-ivory/70">{d}</p></div>)}</div></div></section>
    ...
    <section className="relative bg-ivory py-24 sm:py-32"><div className="section-shell grid items-center gap-14 lg:grid-cols-2"><div><h2 className="text-balance text-4xl font-display text-gold sm:text-5xl">Por que devo ler este livro?</h2><div className="mt-7 space-y-4 max-w-xl leading-8 text-charcoal/82"><p>Porque a dor de uma despedida não precisa ser enfrentada sozinho. Porque existe conforto na Palavra de Deus para quem sente saudade. E porque, mesmo em meio ao luto, ainda é possível encontrar esperança.</p><p>Se você busca um caminho de fé para atravessar essa dor — ou quer oferecer esse caminho a alguém que ama — estas páginas foram escritas para caminhar ao seu lado, com a Palavra como companhia.</p></div><div className="mt-10 space-y-8"><blockquote><p className="text-2xl text-navy">"Jesus chorou."</p><cite className="mt-2 block text-xs not-italic uppercase tracking-[0.18em] text-gold">João 11,35</cite></blockquote><blockquote><p className="text-2xl text-navy">"Eu sou a ressurreição e a vida."</p><cite className="mt-2 block text-xs not-italic uppercase tracking-[0.18em] text-gold">João 11,25</cite></blockquote><blockquote className="border-l border-gold pl-6"><p className="text-3xl leading-snug text-gold">"Temos esta esperança como âncora da alma, firme e segura."</p><cite className="mt-3 block text-xs not-italic uppercase tracking-[0.18em] text-charcoal/60">Hebreus 6,19</cite></blockquote></div></div><img src={bibleLantern} alt="Bíblia aberta ao lado de uma lanterna acesa" loading="lazy" width={1600} height={1072} className="aspect-[4/3] w-full object-cover shadow-xl ring-1 ring-gold/30"/></div></section>
    ...
    <section className="bg-ivory py-24 sm:py-32"><div className="section-shell"><div className="max-w-2xl"><Eyebrow>Um encontro possível</Eyebrow><h2 className="text-4xl text-navy sm:text-5xl">Talvez este livro seja para você.<br/>Ou para alguém que você ama.</h2><p className="mt-6 max-w-xl leading-8 text-charcoal/80">Talvez você não esteja vivendo essa dor agora — mas é bem provável que conheça alguém que está. Um amigo, um familiar, alguém querido que perdeu uma pessoa especial e ainda procura um pouco de conforto. Oferecer este livro pode ser exatamente esse gesto: uma forma silenciosa de dizer "eu penso em você" a quem você ama.</p></div><div className="mt-14 grid border-t border-gold/40 md:grid-cols-2">{forWhom.map((item,i)=><div key={item} className="flex gap-4 border-b border-gold/25 py-6 md:odd:border-r md:odd:pr-8 md:even:pl-8"><Heart className="mt-1 size-4 shrink-0 text-gold"/><p className="text-base leading-7 text-charcoal/82">{item}</p></div>)}</div></div></section>
      ...
      <div className="py-14"><Eyebrow>Como você vai receber o seu exemplar</Eyebrow><h3 className="text-balance text-3xl sm:text-4xl">Escolha como prefere participar.</h3>
        <div role="radiogroup" aria-label="Forma de recebimento do exemplar" className="mt-10 grid gap-6 md:grid-cols-2">
          <button type="button" role="radio" aria-checked={delivery === "presencial"} onClick={() => setDelivery("presencial")} className={`flex flex-col items-start gap-3 rounded-xl border p-7 text-left shadow-md transition-all hover:-translate-y-0.5 hover:shadow-xl ${delivery === "presencial" ? "border-gold bg-gradient-to-b from-gold/15 to-transparent shadow-lg" : "border-gold/30 bg-gradient-to-b from-navy-soft to-navy hover:border-gold/60"}`}>
            ...
            <p className="text-sm leading-6 text-ivory/72">Retire seu exemplar pessoalmente em 17 de novembro de 2026, às 19h, no Santuário Nossa Senhora das Graças, em Anápolis (GO), e participe da noite de autógrafos com o Pe. Wesley Xavier Ramos.</p>
          </button>
          <button type="button" role="radio" aria-checked={delivery === "correio"} onClick={() => setDelivery("correio")} className={`flex flex-col items-start gap-3 rounded-xl border p-7 text-left shadow-md transition-all hover:-translate-y-0.5 hover:shadow-xl ${delivery === "correio" ? "border-gold bg-gradient-to-b from-gold/15 to-transparent shadow-lg" : "border-gold/30 bg-gradient-to-b from-navy-soft to-navy hover:border-gold/60"}`}>
            ...
            <p className="text-sm leading-6 text-ivory/72">Receba em casa, com entrega em até 10 dias úteis a partir de 17 de novembro de 2026, para quem não puder estar presente.</p>
          </button>
        </div>
      </div>
    ...
    <section className="bg-ivory py-24 sm:py-32"><div className="section-shell mx-auto max-w-2xl"><div className="text-center"><Eyebrow>Perguntas frequentes</Eyebrow><h2 className="text-4xl text-navy sm:text-5xl">Ainda com alguma dúvida?</h2></div><div className="mt-14 grid gap-4">
      <details className="group rounded-xl border border-gold/25 bg-gradient-to-b from-background to-ivory p-5 shadow-sm transition-all hover:-translate-y-0.5 hover:border-gold/50 hover:shadow-lg open:-translate-y-0.5 open:border-gold/50 open:shadow-lg sm:p-6"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-lg text-navy marker:content-none">O livro é físico ou digital?<span className="shrink-0 text-xl text-gold transition-transform group-open:rotate-45">+</span></summary><p className="mt-3 leading-7 text-charcoal/80">É um livro impresso, físico — pensado para ser lido, guardado e relido.</p></details>
      <details className="group rounded-xl border border-gold/25 bg-gradient-to-b from-background to-ivory p-5 shadow-sm transition-all hover:-translate-y-0.5 hover:border-gold/50 hover:shadow-lg open:-translate-y-0.5 open:border-gold/50 open:shadow-lg sm:p-6"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-lg text-navy marker:content-none">Quando e como recebo meu exemplar?<span className="shrink-0 text-xl text-gold transition-transform group-open:rotate-45">+</span></summary><p className="mt-3 leading-7 text-charcoal/80">Há duas formas de participar. Você pode retirar o livro pessoalmente no dia 17 de novembro de 2026, às 19h, no Santuário Nossa Senhora das Graças, em Anápolis (GO), no lançamento oficial, e participar da noite de autógrafos com o Pe. Wesley. Ou, se preferir, pode recebê-lo pelo Correio no conforto da sua casa — a entrega acontece em até 10 dias úteis após o lançamento oficial de 17 de novembro de 2026.</p></details>
      <details className="group rounded-xl border border-gold/25 bg-gradient-to-b from-background to-ivory p-5 shadow-sm transition-all hover:-translate-y-0.5 hover:border-gold/50 hover:shadow-lg open:-translate-y-0.5 open:border-gold/50 open:shadow-lg sm:p-6"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-lg text-navy marker:content-none">Quais as formas de pagamento?<span className="shrink-0 text-xl text-gold transition-transform group-open:rotate-45">+</span></summary><p className="mt-3 leading-7 text-charcoal/80">[Preencher: meios de pagamento aceitos.]</p></details>
      <details className="group rounded-xl border border-gold/25 bg-gradient-to-b from-background to-ivory p-5 shadow-sm transition-all hover:-translate-y-0.5 hover:border-gold/50 hover:shadow-lg open:-translate-y-0.5 open:border-gold/50 open:shadow-lg sm:p-6"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-lg text-navy marker:content-none">Posso presentear alguém com este livro?<span className="shrink-0 text-xl text-gold transition-transform group-open:rotate-45">+</span></summary><p className="mt-3 leading-7 text-charcoal/80">Sim. Ao garantir seu exemplar, você pode indicar que se trata de um presente — é uma forma silenciosa de dizer "estou aqui com você" para quem estiver enfrentando uma despedida.</p></details>
    </div></div></section>

    <section className="relative isolate bg-navy py-28 text-center text-ivory sm:py-40"><img src={horizon} alt="Horizonte iluminado sobre o mar" loading="lazy" width={1920} height={1088} className="absolute inset-0 -z-20 size-full object-cover"/><div className="absolute inset-0 -z-10 bg-navy/70"/><div className="section-shell"><AnchorMark/><h2 className="mx-auto mt-8 max-w-4xl text-balance text-4xl sm:text-6xl">Quando a saudade permanece,<br/><span className="font-script text-[1.2em] text-gold">a esperança</span> pode continuar iluminando o caminho.</h2><p className="mt-7 text-ivory/75">Garanta seu exemplar no pré-lançamento de <em>Quando a Saudade Permanece</em>.</p><div className="mt-9"><PreorderButton label="Garantir meu exemplar"/></div><p className="mt-5 text-xs uppercase tracking-[0.18em] text-ivory/72">Coleção Âncora · Livro I · 17 de novembro, 19h · Santuário Nossa Senhora das Graças, Anápolis (GO)</p></div></section>
